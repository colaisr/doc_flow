...
export default function LoginPage() {
    ...
    onError: (err: any) => {
      ...
      if (detail) {
        if (typeof detail === 'string') {
          ...
          errorDetail = detail[0]?.msg || 'Ошибка валидации'
        } else if (typeof detail === 'object') {
          // Single validation error object
          errorDetail = detail.msg || detail.message || 'Ошибка валидации'
        }
      }
  ...
  const verifyMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      // Handle different error formats from FastAPI
      let errorDetail = 'Неверный код подтверждения'
      const detail = err.response?.data?.detail
      ...
      if (detail) {
        if (typeof detail === 'string') {
          ...
          errorDetail = detail[0]?.msg || 'Неверный код подтверждения'
        } else if (typeof detail === 'object') {
          errorDetail = detail.msg || detail.message || 'Неверный код подтверждения'
        }
      }
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8">
            ...
            <p className="text-gray-600 mb-8 text-center">
              Войдите в систему для управления лидами
            </p>
                            ...
                            try {
                              ...
                              if (detail) {
                                if (typeof detail === 'string') {
                                  ...
                                  message = detail[0]?.msg || 'Ошибка отправки письма'
                                } else if (typeof detail === 'object') {
                                  message = detail.msg || detail.message || 'Ошибка отправки письма'
                                }
                              }
